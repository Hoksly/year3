...
def des_ecb_hash_from_string(s):
    return encrypt(s, KEY)

def des_ecb_hash_from_file(file):
    with open(file, 'r') as f:
        return des_ecb_hash_from_string(f.read())
    

def main():
  global KEY
  parser = argparse.ArgumentParser(description='Compute DES-ECB hash of a string or file.')
  ...
  parser.add_argument("-k", "--key", type=str, help="DES key (default: F7AC7D10C9F8145F)", default="F7AC7D10C9F8145F")
  ...
  if args.string:
      print(f"DES-ECB hash of '{args.string}': {des_ecb_hash_from_string(args.string)}")
  elif args.file:
      print(f"DES-ECB hash of file '{args.file}': {des_ecb_hash_from_file(args.file)}")
